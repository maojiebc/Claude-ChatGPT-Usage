...
- **Claude 精简浮窗**：默认用 `5h / 7d / F5` 三行胶囊显示剩余比例，鼠标移入后平滑展开完整进度、倒计时和重置时间；条目图标区分额度类型，颜色按剩余量走绿 / 琥珀 / 红三档健康度。
- **定制图标资产**：Claude 展开态使用设计稿风格的闪电、时钟、日历、大脑、刷新、设置与关闭图标，并内嵌为透明高清资源。
